...
public class TERenderer {
    ...
    /** only displays a  by 7 square of the world centered around the avatar */
    public void renderAvatarFrame(TETile[][] world, int xCord, int yCord) {
        /** didn't handle edge cases with if xcord - is out of frame */
        int startXCord = xCord - 2;
        int endXCord = xCord + 3;
        int startYCord = yCord - 2;
        int endYCord = yCord + 3;
        if (startXCord < 0) {
            startXCord = 0;
        }
        if (endXCord > world.length) {
            endXCord = world.length;
        }
        if (startYCord < 0) {
            startYCord = 0;
        }
        if (endYCord > world[0].length) {
            endYCord = world[0].length;
        }
        StdDraw.clear(new Color(0,0,0));
        for (int x = startXCord; x < endXCord; x += 1) {
            for (int y = startYCord; y < endYCord; y += 1) {
                if (world[x][y] == null) {
                    throw new IllegalArgumentException("Tile at position x=" + x + ", y=" + y
                            + " is null.");
                }
